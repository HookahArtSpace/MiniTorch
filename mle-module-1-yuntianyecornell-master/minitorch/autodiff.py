    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        for i, input in enumerate(inputs):
            if not is_constant(input):
                ...
                out.append([inputs[i], d[i]])
        # print(out)
        return out


def is_constant(val):
    # print('not a variable inside', not isinstance(val,Variable))
...
def backpropagate(variable, deriv):
    ...
    for item in sorted:
        ...
        if item.is_leaf():
            item.accumulate_derivative(dict[item.unique_id])
        else:
            if dict:
                outputs = item.history.backprop_step(dict[item.unique_id])
            else:
                outputs = item.history.backprop_step(deriv)

            for vari, deri in outputs:
                # print(item.unique_id, vari.unique_id,deri)
                if vari.unique_id in dict:
                    dict[vari.unique_id] += deri
                else:
                    dict.update({vari.unique_id: deri})

    # print([var.derivative for var in t_sorted])
