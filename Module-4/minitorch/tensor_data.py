...
def index_to_position(index, strides):
    ...
    """
    # index * strides += ==> position
    position = 0
    for i, strides in zip(index, strides):
        position += i * strides
    return position
...
def broadcast_index(big_index, big_shape, shape, out_index):
    ...
    for i in range(len(shape)):
        difference = len(big_shape) - len(shape) + i
        if shape[i] > 1:
            out_index[i] = big_index[difference]
        else:
            out_index[i] = 0
...
def shape_broadcast(shape1, shape2):
    ...
    if len(shape1) - len(shape2) > 0:
        shape2 = ((1,) * (len(shape1) - len(shape2))) + shape2
    elif len(shape1) - len(shape2) < 0:
        shape1 = ((1,) * abs(len(shape1) - len(shape2))) + shape1
    shape1 = list(shape1)
    shape2 = list(shape2)
    for i in range(len(shape1) - 1, -1, -1):
        if shape1[i] == shape2[i]:
            ...
            shape2[i] = max(shape1[i], shape2[i])
        else:
...
class TensorData:
    ...
    def permute(self, *order):
        ...
        # change positions based on order
