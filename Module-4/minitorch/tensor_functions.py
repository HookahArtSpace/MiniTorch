    ...
    class Backend:
        ...
        class Sigmoid(Function):
            ...
            def backward(ctx, grad_output):
                b = ctx.saved_values
                return mul_zip(
                    grad_output, mul_zip(b, add_zip(tensor([1]), neg_map(b)))
                )
        ...
        class Permute(Function):
            ...
            def backward(ctx, grad_output):
                ...
                new_order = []
                for index, value in sorted(enumerate(order)):
                    new_order.append((index, value))
                new_order.sort(key=lambda new_order: new_order[1])
                new_order = [value for value, index in new_order]
