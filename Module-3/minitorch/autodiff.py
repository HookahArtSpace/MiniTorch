...
class FunctionBase:
    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        """
        # history = []
        # temp = cls.backward(ctx, d_output)
        # for idx, item in enumerate(inputs):
        #     if not is_constant(item):
        #         history.append([item, temp[idx]])
        # return history
...
def backpropagate(variable, deriv):
    ...
    # dict = {}
    # for item in t_sorted:
    #     if not item.is_leaf():
    #         if dict:
    #             outputs = item.history.backprop_step(dict[item.unique_id])
    #         else:
    #             outputs = item.history.backprop_step(deriv)
    #         for vari, derivatives in outputs:
    #             if vari.unique_id in dict:
    #                 dict[vari.unique_id] += derivatives
    #             else:
    #                 dict.update({vari.unique_id: derivatives})
    #     else:
