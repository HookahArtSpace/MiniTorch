...
def shape_broadcast(shape1, shape2):
    ...
    """
    # check broadcastibility
    end = min(len(shape2), len(shape1))
    i = 1
    while i <= end:
        ...
        i += 1
    out_shape = [0 for i in range(max(len(shape2), len(shape1)))]
    for i in range(1, len(out_shape) + 1):
        if i <= len(shape1) and i <= len(shape2):
            out_shape[-i] = max(shape2[-i], shape1[-i])
        elif i > len(shape1):
...
class TensorData:
    ...
    def permute(self, *order):
        ...
        strides = tuple([self.strides[index] for index in order])
